...
def load_design(design_file_path, generate_example=False):
    ...
    if os.path.exists(design_file_path):
        ...
        return Design(__read_layers(cp), __read_smells(cp))
    elif generate_example:
        __write_layers(cp, default_design.layers)
        __write_layers(cp, exmaple_layers)

        __write_smells(cp, default_design.smells)
        __write_smells(cp, example_smells)

        __write_design(cp, design_file_path)
