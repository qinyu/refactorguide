...
def console_markdown(module_dict):
    for m, pkg_dict in module_dict.items():
        ...
        for p, classes in pkg_dict.items():
            ...
            markdown+="\n"+"dependency:\n"
            for file in classes:
                markdown+="".join([uml_class_format.format(dep_file.package+ "." +dep_file.name)for dep_file in file.suspicious_dependencies])
            markdown+="\n"+"usage:\n"
            for file in classes:
                markdown+="".join([uml_class_format.format(usages_file.package+ "." +usages_file.name)for usages_file in file.suspicious_usages])
...
def get_markdown(module_name,package_name,classes):
    ...
    result+="".join([uml_class_format.format(file.package+ "." +file.name) for file in classes])
    return result
